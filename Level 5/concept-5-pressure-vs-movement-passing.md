# Pressure vs Movement Passing

---

## Definition

Pressure passing and movement passing are the two fundamental approaches to defeating the guard. Pressure passing uses weight, control, and incremental advancement to crush the guard structure and grind through it. Movement passing uses speed, angle changes, and redirection to go around the guard before the bottom player can adjust. Every guard pass in jiu jitsu is either a pressure pass, a movement pass, or a combination of both.

---

## Why It Matters

Without understanding these two categories, the student collects individual passes without understanding why some work in certain situations and others do not. A pressure pass against a fast, mobile guard player may fail because they re-guard before the slow grind finishes. A movement pass against a heavy, grippy guard player may fail because they anchor the passer and prevent the speed needed to go around.

Understanding the two categories gives the student:
...
Pressure passes go through the guard slowly. Movement passes go around the guard quickly. Know when to use each, and be able to switch between them.
...
- **Constant contact**: The passer's body stays connected to the guard player at all times. There is no space between the bodies.
...
Movement passes work by changing angles and direction faster than the guard player can adjust their defensive structure. The passer redirects the guard player's legs, releases contact momentarily, and sprints past the legs to establish control on the other side before the guard player can reposition.
...
| ------------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Guard player is mobile and agile      | Pressure                    | Slow them down; take away space they need to be mobile                                                                     |
| Guard player is strong and grippy     | Movement                    | Do not fight their grips head-on; use speed to make grips irrelevant                                                       |
| Guard player is fatigued              | Either (pressure preferred) | Fatigued opponents struggle against weight; pressure accelerates the fatigue                                               |
| Guard player is fresh and reactive    | Movement                    | Fresh opponents can hold frames under pressure; use speed to bypass reactions                                              |
| Passer has dominant grips established | Pressure                    | Grips are already in place; use them to control and grind                                                                  |
| Passer's grips are broken             | Movement                    | Without grips, pressure has no anchor; use speed while grips are being reset                                               |
| Half guard / deep half                | Pressure                    | Close-range guards favor pressure because there is not enough space for movement                                           |
| Open guard / spider / lasso           | Movement or combination     | Long-range guards create too much distance for immediate pressure; use movement to close distance, then pressure to finish |
| Opponent is much larger               | Movement                    | Pressure against a larger opponent is less effective; use speed differential                                               |
| Opponent is much smaller              | Pressure                    | Weight advantage makes pressure devastating; smaller opponents are harder to catch with speed                              |

**The combination: pressure to movement, movement to pressure.**
...
This is the advanced application: use pressure to set up movement, and movement to set up pressure. The guard player cannot defend both simultaneously. If they hunker down to survive pressure, they cannot track movement. If they stay light and mobile to track movement, they cannot survive pressure.

**Matching the approach to the pass.**

The student does not need to learn separate passes for each approach. The same pass can often be executed with pressure or movement emphasis:

- **Knee cut**: Can be a slow pressure grind (heavy hip pressure, slow slide through) or a fast movement cut (quick angle change, sprint to the other side)
- **Torreando**: Primarily a movement pass, but can be slowed down with heavy downward pressure on the legs before the throw
- **Stack pass**: Primarily a pressure pass, but the directional change (circling the legs to one side) is a movement component

Understanding pressure vs movement allows the student to modify any pass they know to fit the situation.

---

## Key Principles

- Every pass is either pressure, movement, or a combination
- Pressure goes through the guard slowly with weight and control; movement goes around the guard quickly with speed and angles
- Choose the approach based on how the guard player is defending, not based on personal preference
- Pressure creates openings for movement; movement creates openings for pressure
- The best passing game includes both categories and the ability to switch between them
- Failed passes often fail because the wrong category was chosen, not because the technique was wrong
- Pressure is low-risk and grinds; movement is higher-risk and explosive
...
- Guard player starts in a random guard (closed, open, half, butterfly, spider)
- Passer has 5 seconds to read the position and declare "pressure" or "movement"
- Explain the reasoning for the choice
- Then attempt the pass using the declared approach
- Builds the habit of reading before acting
