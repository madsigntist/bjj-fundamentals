...
Guard is not one wall. It is a series of walls. When the first falls, the next is already in place.
...
1. Layer 1 breaks → drop to Layer 2
2. From Layer 2, fight to re-establish Layer 1 (re-insert feet on hips)
3. If Layer 2 breaks → drop to Layer 3
4. From Layer 3, fight to re-establish Layer 2 (re-insert knee or shin)
5. If Layer 3 fails → guard is passed → switch to escape mode (Level 2 skills)
...
| Layer                | Primary Tools                                     | Range        | Goal                                                       |
| -------------------- | ------------------------------------------------- | ------------ | ---------------------------------------------------------- |
| Layer 1: Feet/ankles | Feet on hips, feet on biceps, long-range hooks    | Long-medium  | Maintain maximum distance                                  |
| Layer 2: Knees/shins | Knee shield, butterfly hooks, shin frames         | Medium-close | Block advance, create sweep opportunities, recover Layer 1 |
| Layer 3: Hips/frames | Hip escape, forearm frames, elbow-knee connection | Close        | Create enough space to re-insert legs, survive             |
...
### Open Guard Under Passing Pressure

- **Layer 1**: Feet on hips, tracking the passer's movement with hip scoots. Sleeve grips preventing arm control.
- **Layer 1 breaks**: Passer strips both feet and steps to the side.
- **Layer 2 activates**: Insert knee shield across the body, or hook the near leg with butterfly hook. Re-establish a grip.
- **Layer 2 holds**: Use knee shield to push the passer back, then re-insert feet on hips (recover Layer 1).
- **Layer 2 breaks**: Passer flattens the knee shield and drives chest-to-chest.
- **Layer 3 activates**: Forearm frame against the neck or shoulder. Hip escape to create distance. Elbow connects to top knee.
- **Layer 3 goal**: Create enough space to re-insert a knee between bodies → back to Layer 2.

### Half Guard Under Smash Pressure

- **Layer 2 starting position**: Knee shield in place, underhook fighting for position.
- **Layer 2 breaks**: Passer crossfaces and flattens the knee shield. Underhook lost.
- **Layer 3 activates**: Frame with forearm against the crossface. Hip escape away. Fight to re-insert knee or free the trapped leg.
- **Recovery**: If space is created → re-insert knee shield (back to Layer 2). If leg is freed → transitional feet on hips (Layer 1) or full guard recovery.

### Closed Guard to Open Guard Transition

- **Layer 1 (closed guard equivalent)**: Legs locked around waist. Posture broken.
- **Transition**: Opponent postures up and breaks the closed guard.
- **Layer 1 (open guard)**: Immediately place feet on hips before they can pass. Establish sleeve grips.
- **If slow**: The passer rushes past the legs → Layer 2 (knee shield, butterfly) or Layer 3 (frames, hip escape).

---

## Simple Mental Model

Three fences around a house. The passer must climb each one. If they get over the first, the second is already there. If they get over the second, the third is waiting. Your job is to rebuild each fence as fast as they climb it.
...
- Start with feet on hips (Layer 1)
- Partner strips your feet → immediately transition to knee shield (Layer 2)
- Partner flattens knee shield → immediately transition to hip escape and frames (Layer 3)
- From Layer 3, recover back to Layer 2, then Layer 1
- Repeat the full descent and recovery 5 times per round
